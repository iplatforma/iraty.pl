...
<section id="banner" class="full">
	<div class="justify">
    	<header><h2 class="less">Edytuj stronę</h2></header>
	...
	<div class="justify">
        ...
        <form class="form" action="<?=site_url('podstrona/zapisz')?>" method="post">
            <div class="input">
                ...
                <input type="text" name="header" placeholder="Nazwa strony" value="<?=$wpis->header?>" required>
            </div>
            <div class="input">
                <label>Przenieś do podrzędnej</label>
                <select name="parent">
                	<option value="">--- nie przenoś ---</option>
                 <? foreach(modules::run('podstrona/pobierz')->result() as $parent) { if($parent->parent == NULL and $parent->menu == '1') { if($parent->parent != $wpis->id) { ?>
                    <option value="<?=$parent->id?>"<?=$this->selected($wpis->parent,$parent->id)?>><?=$parent->header?></option>
				<? }}} ?>
                </select>
            </div>
            <div class="input">
