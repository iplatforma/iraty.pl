...
<section id="banner" class="full">
	<div class="justify">
    	<header><h2 class="less">Zarządzanie aktualnościami</h2></header>
...
<section id="main" class="padding more">
	<div class="justify">
    	<aside class="admin">
        	<a href="<?=site_url('aktualnosci/dodaj')?>" class="button green icon"><i class="fa fa-plus"></i> Dodaj nowy wpis</a>
        </aside>
        ...
        <table class="dane">
            ...
            <tr<?=$this->statusview($dane->status)?>>
            	<td><? if($dane->src) { ?><img src="<?=assets('img/aktualnosci/thumb/'.$dane->src)?>"><? } ?></td>
            	<td><?=$dane->title?></td>
                ...
                <td class="settings">
                	<aside class="admin">
                        ...
                        <a href="<?=site_url('aktualnosci/edytuj/'.$dane->id)?>" title="Edytuj wpis"><i class="fa fa-edit"></i></a>
		...
		<? foreach($pobierz->result() as $dane) { ?>
            ...
            <div id="aktywuj-<?=$dane->id?>" class="hide">
                ...
                <p><a href="<?=site_url('aktualnosci/status/'.$dane->id.'/1')?>">Tak, aktywuj wpis!</a></p>
            </div>
            <? } else { ?>
            <div id="dezaktywuj-<?=$dane->id?>" class="hide">
                ...
                <p><a href="<?=site_url('aktualnosci/status/'.$dane->id.'/0')?>">Tak, dezaktywuj wpis!</a></p>
            </div>
            <? } ?>
            <div id="dodaj-glowne-<?=$dane->id?>" class="hide">
                <header><h3>Ustaw zdjęcie</h3></header>
                <? if($dane->src) { ?><img src="<?=assets('img/aktualnosci/thumb/'.$dane->src)?>" style="max-width:100%;"><? } ?>
                <form action="<?=site_url('aktualnosci/src/dodaj')?>" enctype="multipart/form-data" method="post">
                ...
                <? if($dane->src) { ?><p class="full">lub <a href="<?=site_url('aktualnosci/src/usun/'.$dane->id)?>" title="Usuń zdjęcie główne">usuń zdjęcie</a>.</p><? } ?>
            </div>
           <div id="zmien-data-<?=$dane->id?>" class="hide">
                <header><h3>Zmień priorytet</h3></header>
                <form action="<?=site_url('aktualnosci/kolejnosc/zapisz')?>" enctype="multipart/form-data" method="post">
        	...
        	<div class="hide" id="usun-<?=$dane->id?>">
                ...
                <p><a href="<?=site_url('aktualnosci/usun/'.$dane->id)?>">Tak, usuń wpis!</a></p>
            </div>
        <? } ?>
		<? if($paginate) { ?><div class="pagination justify"><?=$paginate?></div><? } ?>
    </div>
