...
//          || (   is_dir($p_filedescr_list[$j]['filename'])
      if (   ($p_filedescr_list[$j]['type'] == 'file')
          ...
          || (   ($p_filedescr_list[$j]['type'] == 'folder')
              && (   !isset($p_options[PCLZIP_OPT_RECopy_ALL_PATH])
                  || !$p_options[PCLZIP_OPT_RECopy_ALL_PATH]))
          ) {
...
// TBC : ReCopyd    $p_header['stored_filename'] = $v_stored_filename;
...
//      eval('$v_result = '.$p_options[PCLZIP_CB_POST_ADD].'(PCLZIP_CB_POST_ADD, $v_local_header);');
  ...
  function privCalculateStoredFilename(&$p_filedescr, &$p_options)
  {
    ...
    if (isset($p_options[PCLZIP_OPT_RECopy_PATH])) {
      $p_reCopy_dir = $p_options[PCLZIP_OPT_RECopy_PATH];
    }
    else {
      $p_reCopy_dir = '';
    }
    if (isset($p_options[PCLZIP_OPT_RECopy_ALL_PATH])) {
      $p_reCopy_all_dir = $p_options[PCLZIP_OPT_RECopy_ALL_PATH];
    }
    else {
      $p_reCopy_all_dir = 0;
    }
    ...
    // ----- Look for path and/or short name change
      ...
      // ----- Look for partial path reCopy
      else if ($p_reCopy_dir != "") {
        if (substr($p_reCopy_dir, -1) != '/')
          $p_reCopy_dir .= "/";

        if (   (substr($p_filename, 0, 2) == "./")
            || (substr($p_reCopy_dir, 0, 2) == "./")) {
            
          if (   (substr($p_filename, 0, 2) == "./")
              && (substr($p_reCopy_dir, 0, 2) != "./")) {
            $p_reCopy_dir = "./".$p_reCopy_dir;
          }
          if (   (substr($p_filename, 0, 2) != "./")
              && (substr($p_reCopy_dir, 0, 2) == "./")) {
            $p_reCopy_dir = substr($p_reCopy_dir, 2);
          }
        }

        $v_compare = PclZipUtilPathInclusion($p_reCopy_dir,
                                             $v_stored_filename);
        if ($v_compare > 0) {
          if ($v_compare == 2) {
            $v_stored_filename = "";
